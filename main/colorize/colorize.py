...
class Colroizer:
    def __init__(self, height=480, width=600):
        ...
        prototxt_file = join(dirname(__file__), "model/colorization_deploy_v2.prototxt")
        caffemodel_file = join(dirname(__file__), "model/colorization_release_v2.caffemodel")
        self.colorModel = cv2.dnn.readNetFromCaffe(prototxt_file, caffeModel=caffemodel_file)

        npy_file = join(dirname(__file__), "model/pts_in_hull.npy")
        clusterCenters = np.load(npy_file)
    ...
    def processImage(self, imgName):
        ...
        frame_jpg = cv2.imencode('.jpg', self.imgFinal)
        imageFile = ContentFile(frame_jpg[1])
